...
(() => {
  var t = (() => {
    ...
    ; hljs.registerLanguage("undefined", t)
})();
...
function addCopyPasteButtons() {
  ...
  for (var i = 0; i < numberOfSnippets; i++) {
    // if copy button isn't already added, create it
    if (snippets[i].getElementsByClassName('hljs-copy').length > 0) {
      continue;
    }
    code = snippets[i].getElementsByTagName('code')[0].innerText;
    snippets[i].classList.add('hljs'); // append copy button to pre tag

    snippets[i].innerHTML = '<button class="btn hljs-copy"><i class="mdi mdi-content-copy"></i></button>' + snippets[i].innerHTML; // append copy button
